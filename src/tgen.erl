...
process_json(#tgen{name = GName, module = Module, sha = SHA}, Content) ->
    case jsx:decode(Content, [return_maps, {labels, attempt_atom}]) of
        _JSON = #{exercise := GName, cases := Cases0, version := TestVersion} ->
            ...
            {TestModuleName, TestModuleContent} = generate_test_module(SHA, Module, binary_to_list(GName), TestImpls1, binary_to_list(TestVersion)),
...
generate_test_module(SHA, Module, ModuleName, Tests, TestVersion) ->
    SluggedModName = slugify(ModuleName),
    ...
    Abstract = [
		...
		[
			...
			"% https://github.com/exercism/problem-specifications/raw/" ++ SHA ++ "/exercises/" ++ ModuleName  ++ "/canonical-data.json",
